...
// language=HTML
function outputOWM(weatherArrSwitcher){
     ...
     forecastScroller.innerHTML += `
         <div class="forecast flex-col">
          <div class="date-container flex-row">
              <img src="http://openweathermap.org/img/wn/${weatherArrSwitcher.daily[i].weather[0].icon}@2x.png" class="owm-icon">
       <p class="forecast-day">
           ${new Date(weatherArrSwitcher.daily[i].dt).toLocaleString('en-us', {weekday: 'short'})}
           </p>
       </div>
       <div class="forecast-hilo flex-row">
           <p class="forecast-lo">Lo: ${weatherArrSwitcher.daily[i].temp.min} /</p>
           <p class="forecast-hi">Hi: ${weatherArrSwitcher.daily[i].temp.max}</p>
       </div>
       <div class="description-wrapper flex-col">
           <p class="description-title">Description:</p>
           <p class="description-output">${weatherArrSwitcher.daily[i].weather[0].description}</p>
       </div>
       <div class="humidity-wrapper flex-row">
           <p class="humidity-title">Humidity:</p>
           <p class="humidity-output">${weatherArrSwitcher.daily[i].humidity}</p>
       </div>
       <div class="wind-speed-wrapper flex-row">
           <p class="wind-speed-title">Wind Speed:</p>
           <p class="wind-speed-output">${weatherArrSwitcher.daily[i].wind_speed}</p>
       </div>
         </div>
     `
    }
